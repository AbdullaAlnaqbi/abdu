    ...
    def chase(self):
        ...
        while 1:
            ...
            try:
                ...
                y = self.detected_object[1]
                detected_x_class = x.Class
                detected_y_class = y.Class
                xmin = x.xmin
                xmax = x.xmax
                ymin = y.ymin
                ymax = y.ymax
                print('box middle is {} and the x_class is {} box middle is {} and the y_class is {} '.format(
                    (xmin+xmax)/2, detected_x_class, (ymin+ymax)/2, detected_y_class))
                ...
                # roll forward when an object is detected in UPPER_PARTITION
                elif (xmin+xmax)/2 < UPPER_PARTITION and detected_flag is True:
                    print("roll up")
                    upper_flag = True
                    lower_flag = False
                ...
                # roll right
                elif (xmin+xmax)/2 > LOWER_PARTITION and detected_flag is True:
                    lower_flag = True
                    upper_flag = False
                    print("roll down")
                    self.send_ned_velocity(FOREWARD_SPEED,0,0,FOREWARD_DURATION)
